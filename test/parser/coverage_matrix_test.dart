...
void main() {
  ...
  test('every catalogue entry with a nutrient record (direct or via '
    ...
    for (final entry in withNutrition) {
      ...
      if (result.items.length != 1) {
        failures.add(
          '${entry.id}: expected exactly one item, got ${result.items.length}',
        );
        continue;
      }
      ...
      if (item.confidence == ParseConfidence.incomplete ||
          item.nutrition == null) {
        failures.add(
          '${entry.id}: confidence=${item.confidence} nutrition=${item.nutrition}',
        );
      }
    }
  ...
  group('representative own-data entries across newly covered categories', () {
    const samples = {
      ...
      'lamb': 'meat_poultry',
      'duck': 'meat_poultry',
      'mackerel': 'fish_seafood',
      'herring': 'fish_seafood',
      'shrimp': 'fish_seafood',
      'crab': 'fish_seafood',
      'smoked_salmon': 'fish_seafood',
      'feta': 'dairy_alternative',
      'parmesan': 'dairy_alternative',
      'butter': 'dairy_alternative',
      'whole_milk': 'dairy_alternative',
      'sour_cream': 'dairy',
      'bagel': 'grain_bakery',
      'croissant': 'grain_bakery',
      'wrap': 'grain_bakery',
      'couscous': 'grain_bakery',
      'quinoa': 'grain_bakery',
      'chickpeas': 'legume_nut_seed',
      'tofu': 'legume_nut_seed',
      'hummus': 'legume_nut_seed',
      ...
      'coca_cola': 'drink',
      'coke_zero': 'drink',
      'ketchup': 'sauce_condiment_fat',
      'pesto': 'sauce_condiment_fat',
      'kebab': 'prepared_meal',
      'fried_rice': 'prepared_meal',
      'sushi': 'prepared_meal',
      'biscuit': 'snack_dessert',
      'ice_cream': 'snack_dessert',
      'chocolate_bar': 'snack_dessert',
    };
    samples.forEach((foodId, category) {
      test('"$foodId" ($category) has its own real nutrient record', () {
        final entry = parser.catalogue.entries.firstWhere(
          (e) => e.id == foodId,
        );
  ...
  group('generic-parent chains added in this coverage pass', () {
    ...
    const chains = {
      'back_bacon': 'bacon',
      'hot_dog_sausage': 'sausage',
      'smoked_mackerel': 'mackerel',
      'smoked_trout': 'trout',
      'king_prawn': 'shrimp',
      'basmati_rice': 'rice',
      'penne': 'pasta',
    ...
    chains.forEach((subtypeId, parentId) {
      test(
        '"$subtypeId" borrows real data from "$parentId" at Low confidence',
        () {
  ...
  group('subtypes upgraded to their own real record', () {
    const ownData = {
      ...
      'duck_breast': 123.0,
      'cooked_ham': 172.0,
      'pickled_herring': 262.0,
      'white_bread': 238.0,
      'black_beans': 132.0,
      'red_cabbage': 31.0,
      'iceberg_lettuce': 14.0,
      'red_pepper': 26.0,
      'green_tea': 1.0,
      'instant_coffee': 3.0,
      'red_wine': 85.0,
    ...
    ownData.forEach((foodId, kcalPer100g) {
      test('"$foodId" resolves with its own real data at High confidence', () {
        final item = parser
            .parse('100g ${foodId.replaceAll('_', ' ')}')
            .items
            .single;
  ...
  group('nut-specific handful portion', () {
    for (final foodId in ['almonds', 'cashews', 'walnuts', 'peanuts']) {
      test('"handful of $foodId" resolves at 30g, medium confidence', () {
        final item = parser
            .parse('handful of ${foodId.replaceAll('_', ' ')}')
            .items
            .single;
        expect(item.canonicalId, foodId);
