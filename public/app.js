...
const api = async (path, options = {}) => {
  const response = await fetch(`/admin/api/${path}`, { ...options, headers: {'authorization': `Bearer ${$('token').value}`, 'content-type': 'application/json', ...(options.headers || {})} });
...
async function load() {
  try { data = await api('content'); $('status').textContent = 'Connected'; render(); }
...
window.publishDraft = async id => {
  const type = $(`type-${id}`).value; const ids = $(`ids-${id}`).value.split(',').map(x => x.trim()).filter(Boolean);
  if (type !== 'all' && !ids.length) return alert('Enter at least one target ID.');
  await api('publish', {method:'POST', body:JSON.stringify({draftId:id,target:{type,ids}})}); await load();
};
window.withdraw = async id => { if (confirm('Withdraw this release from future device updates?')) { await api('unpublish', {method:'POST', body:JSON.stringify({releaseId:id})}); await load(); } };
